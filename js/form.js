...
const сommentInput = document.querySelector('.text__description');
const hashtagsReg =  /^#[A-Za-zА-Яа-яЁё0-9]{1,19}$/;
...
const testHashtagsArrayOnReg = (hashtagsArray) => {
  for (let hashtagIndex = 0; hashtagIndex < hashtagsArray.length; hashtagIndex++) {
    if (!hashtagsReg.test(hashtagsArray[hashtagIndex])) {
      return false;
    }
  }
  return true;
...
const initUploadForm = () => {
  ...
  сommentInput.addEventListener('keydown', (evt) => {
    stopEscEvent(evt);
  });
  ...
  hashtagInput.addEventListener('input', handleUserHashtagInput);
};
